...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (!categoryData) {
      res.status(404).json({ message: 'No categories or products can be found.'})
      return;
    };
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!singleCategoryData) {
      res.status(404).json({ message: 'No category or product information associated with this id.'})
      return;
    };
